import { invokeAgent, type AgentConfig } from "./agents/cli-agent.js";
import { ConversationHistory } from "./history.js";
import {
  formatBuildersSpec,
  queueSpec,
  type SpecData,
} from "./spec-formatter.js";
...
export class Orchestrator {
  ...
  private async runDiscuss(): Promise<void> {
    for (const agentKey of this.agentOrder) {
      const agent = this.config.agents[agentKey];
      const context = this.history.buildContext(
        agent.label,
        this.mode,
        this.activeRepo,
      );
      ...
      try {
        ...
        this.history.add(
          "system",
          `${agent.label} failed: ${err.message}`,
        );
      }
    }
  }
   ...
   */
  private async runDraftReview(): Promise<void> {
    ...
    for (let round = 1; round <= MAX_ROUNDS; round++) {
      ...
      const draftContext = this.history.buildDraftContext(
        drafterConfig.label,
        draftInstruction,
        this.activeRepo,
      );
      ...
      let stop = startSpinner(
        `${drafterConfig.label} (draft round ${round})`,
        drafterConfig.color,
      );
      try {
        ...
        printAgent(
          `${drafterConfig.label} [Draft R${round}]`,
          drafterConfig.color,
          draft,
        );
      } catch (err: any) {
      ...
      const reviewContext = this.history.buildDraftContext(
        reviewerConfig.label,
        reviewInstruction,
        this.activeRepo,
      );

      stop = startSpinner(
        `${reviewerConfig.label} (review round ${round})`,
        reviewerConfig.color,
      );
      try {
        ...
        printAgent(
          `${reviewerConfig.label} [Review R${round}]`,
          reviewerConfig.color,
          review,
        );
        ...
        if (review.trim().toUpperCase().startsWith("LGTM")) {
          printSystem(
            `${reviewerConfig.label} approved the draft after ${round} round(s).`,
          );
          return;
        }
      } catch (err: any) {
      ...
      if (round === MAX_ROUNDS) {
        printSystem(
          `Reached max ${MAX_ROUNDS} rounds. Use /discuss to continue manually, or /spec to extract what you have.`,
        );
      }
    }
  }
  ...
  async runVolley(topic: string, maxRounds: number = 6): Promise<void> {
    ...
    for (let round = 1; round <= maxRounds; round++) {
      for (const agentKey of this.agentOrder) {
        ...
        const context = this.history.buildVolleyContext(
          agent.label,
          round,
          maxRounds,
          this.activeRepo,
        ...
        const stop = startSpinner(
          `${agent.label} (round ${round}/${maxRounds})`,
          agent.color,
        );
        try {
          const response = await invokeAgent(agent, context);
          ...
          if (CONSENSUS_SIGNALS.some((s) => lower.includes(s))) {
            printSystem(
              `${agent.label} signaled agreement in round ${round}. Volley complete.`,
            );
            // Let the other agent confirm if it hasn't spoken this round
            const remaining = this.agentOrder.slice(
              this.agentOrder.indexOf(agentKey) + 1,
            );
            for (const rKey of remaining) {
              const rAgent = this.config.agents[rKey];
              const rCtx = this.history.buildVolleyContext(
                rAgent.label,
                round,
                maxRounds,
                this.activeRepo,
    ...
    printSystem(
      `Volley finished after ${maxRounds} rounds. Use /spec to extract results, or keep going with /volley.`,
    );
  }
  ...
  /** Save the transcript */
  async saveTranscript(filename?: string): Promise<string> {
