...
export function formatBuildersSpec(data: SpecData): string {
  const lines: string[] = [];
  lines.push("---");
...
export function queueSpec(data: SpecData): { url: string; filename: string } {
  ...
  const result = execSync(
    `echo '${escaped}' | gh gist create --public --filename "${filename}" -`,
...
export function extractSpecFromConversation(
  ...
  const fmMatch = allContent.match(
    /---\s*\nrepo:\s*(\S+)\s*\ntitle:\s*(.+?)\s*\n---/,
  );
  if (fmMatch) {
