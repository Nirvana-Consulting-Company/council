...
export interface HistoryEntry {
  role: string; // "user", agent label, or "system"
  content: string;
  timestamp: Date;
}
...
export class ConversationHistory {
  ...
  buildContext(agentLabel: string, mode: string, repoContext?: string): string {
    ...
    lines.push(
      `You are "${agentLabel}" in a multi-agent planning session with a human and other AI agents.`,
    );
    ...
    lines.push(
      "Respond concisely. Build on what was said. Disagree if you see issues.",
    );
  ...
  buildDraftContext(agentLabel: string, instruction: string, repoContext?: string): string {
    ...
    lines.push(
      `You are "${agentLabel}" in a collaborative draft-review session.`,
    );
    if (repoContext) {
      lines.push(`Target repository: ${repoContext}`);
    }
  ...
  buildVolleyContext(
    ...
    lines.push(
      `You are "${agentLabel}" in an autonomous multi-agent debate/planning session.`,
    );
    lines.push(
      `You are discussing directly with the other agent(s). The human is observing.`,
    );
    ...
    lines.push(
      "Respond to the other agent directly. Challenge, refine, or build on their points.",
    );
    lines.push(
      "Be specific and constructive. If you fully agree on the approach, say 'agreed' clearly.",
    );
    lines.push(
      "Keep responses focused — no need to repeat what's already been established.",
    );
